    ...
    <div class="min-h-full items-center mr-8 py-6 ml-20 border-white shadow-md bg-white/50 rounded-[40px] pt-6 mt-20">
        ...
        <div class="p-6 mx-auto max-w-7xl font-poppins">
                                ...
                                @foreach ($todayTasks as $task)
                                    <div class="mb-2"
                                        ...
                                        <div class="flex items-center ml-3 mr-8">
                                            ...
                                            <span
                                                class="{{ $task->completed ? 'line-through text-gray-500' : '' }} ml-4 flex-1">
                                                {{ $task->title }}
                                            </span>
                                            ...
                                            @if ($task->subtasks->count() > 0)
                                                <button class="ml-2 text-gray-500 subtask-toggle"
                                                    data-task="{{ $task->id }}">
                                                    <svg class="w-6 h-6 transition-transform duration-200 subtask-arrow hover:scale-110"
                                                        fill="#6b7280" viewBox="0 0 20 20"
                                            ...
                                            <div class="relative ml-2">
                                                <button class="text-gray-500 task-menu-btn hover:text-gray-700"
                                                    data-task="{{ $task->id }}">
                                                    <svg class="w-3 h-3 transition-transform duration-200 hover:scale-110"
                                                ...
                                                <div class="absolute right-0 z-50 hidden w-48 mt-1 shadow-xl rounded-xl bg-[#0C1F3B] task-menu"
                                                    ...
                                                    <button
                                                        class="flex items-center w-full gap-2 px-4 py-2 text-sm font-medium text-left text-white rounded-t-xl hover:bg-gray-600 details-btn"
                                                        data-task="{{ $task->id }}">
                                                    ...
                                                    <button
                                                        class="flex items-center w-full gap-3 px-3 py-2 text-sm font-medium text-left text-white hover:bg-gray-600 rename-btn"
                                                        data-task="{{ $task->id }}">
                                                    ...
                                                    <button
                                                        class="flex items-center w-full gap-3 px-3 py-2 text-sm font-medium text-left text-white hover:bg-gray-600 duplicate-btn"
                                                        data-task="{{ $task->id }}">
                                                    ...
                                                    <button
                                                        class="flex items-center w-full gap-3 px-3 py-2 text-sm font-medium text-left text-white hover:bg-gray-600 add-subtask-btn"
                                                        data-task="{{ $task->id }}">
                                                        <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none"
                                                            xmlns="http://www.w3.org/2000/svg">
                                                            <path
                                                    ...
                                                    <button
                                                        class="flex items-center w-full gap-3 px-3 py-2 text-sm font-medium text-left text-red-500 rounded-b-xl hover:bg-gray-600 delete-btn"
                                                        data-task="{{ $task->id }}">
                                                        <svg class="w-6 h-6" viewBox="0 0 24 24" fill="none"
                                                            xmlns="http://www.w3.org/2000/svg">
                                                            <path
                                        ...
                                        <div class="hidden ml-14 subtasks" id="subtasks-{{ $task->id }}">
                                            @foreach ($task->subtasks as $subtask)
                                                <div
                                                    class="mb-1 subtask-item {{ $subtask->completed ? 'opacity-50' : '' }}">
                                                    <input type="checkbox"
                                                        class="w-4 h-4 rounded-full subtask-checkbox accent-blue-500"
                                                        data-id="{{ $subtask->id }}"
                                                        {{ $subtask->completed ? 'checked' : '' }}>
                                                    <span
                                                        class="{{ $subtask->completed ? 'line-through text-gray-500' : '' }} ml-2">
                                                        {{ $subtask->title }}
                                                    </span>
                                                </div>
                                            @endforeach
            ...
            <div id="add-task-modal"
                class="fixed inset-0 z-50 hidden w-full h-full overflow-y-auto bg-gray-600 bg-opacity-80 font-poppins">
                <div
                    class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 p-5 shadow-xl rounded-xl w-[600px] bg-[#132C51]">
                    <div class="mt-3">
                        ...
                        <form action="{{ route('tasks.store') }}" method="POST">
                            ...
                            <div class="mb-5">
                                <input placeholder="Title Name" type="text" name="title"
                                    class="w-full px-3 py-2 border text-white border-gray-600 bg-[#0C1F3B] rounded-lg focus:ring-blue-500 focus:border-blue-500"
                                    required>
                            </div>
                            <div class="mb-4">
                                <textarea placeholder="Add Description" name="description"
                                    class="w-full bg-[#0C1F3B] px-3 text-white py-2 border-gray-600 border rounded-lg focus:ring-blue-500 focus:border-blue-500"></textarea>
                            </div>

                            {{-- 2 in 1 (DATE & PRIORITY) --}}
                            <div class="flex flex-wrap justify-center gap-2 mb-5">

                                {{-- DATE INPUT --}}
                                <input type="date" name="due_date"
                                    class="w-[49%] text-white bg-[#0C1F3B] border-gray-600 px-3 py-2 border rounded-lg [color-scheme:dark] focus:ring-blue-500 focus:border-blue-500"
                    ...
                    <div class="mt-3 text-center">
                        ...
                        <div class="flex justify-center gap-6 mt-6 font-medium">
                            <button id="confirm-no"
                                class="px-5 py-1 text-white transition-transform duration-200 bg-gray-500 hover:scale-95 rounded-3xl">No</button>
                            <button id="confirm-yes"
                                class="px-5 py-1 text-white transition-transform duration-200 bg-[#1C427A] hover:scale-110 rounded-3xl">Yes</button>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <form id="rename-form">
                            ...
                            <div class="flex justify-center gap-6 mt-6 font-medium">
                                <button type="button" id="close-rename-modal"
                                    class="px-5 py-1 text-white transition-transform duration-200 bg-gray-500 hover:scale-95 rounded-3xl">Cancel</button>
                                <button type="submit"
                                    class="px-5 py-1 text-white transition-transform duration-200 bg-[#1C427A] hover:scale-110 rounded-3xl">Rename</button>
                            </div>
                        </form>
                    </div>
                </div>
                        ...
                        <form id="add-subtask-form">
                            ...
                            <div class="flex justify-center gap-6 mt-4 font-medium">
                                <button type="button" id="close-add-subtask-modal"
                                    class="px-5 py-1 text-white transition-transform duration-200 bg-gray-500 hover:scale-95 rounded-3xl">Cancel</button>
                                <button type="submit"
                                    class="px-5 py-1 text-white transition-transform duration-200 bg-[#1C427A] hover:scale-110 rounded-3xl">Add</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <div id="delete-confirm-modal"
                ...
                <div
                    ...
                    <div class="mt-3 text-center">
                        <h3 class="mb-4 text-lg font-semibold text-white">Are you sure you want to delete this task?
                        </h3>
                        <div class="flex justify-center gap-6 mt-6 font-medium">
                            <button id="delete-no"
                                class="px-5 py-1 text-white transition-transform duration-200 bg-gray-500 hover:scale-95 rounded-3xl">No</button>
                    ...
                    <div class="mt-3">
                        ...
                        <div class="flex justify-center mt-6 font-medium">
                            <button type="button" id="close-details-modal"
                                class="px-5 py-1 text-white transition-transform duration-200 bg-gray-500 hover:scale-110 rounded-3xl">Close</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        document.querySelectorAll('.subtask-toggle').forEach(button => {
            button.addEventListener('click', function() {
                ...
                if (subtasks.classList.contains('hidden')) {
                    ...
                    // Tambahkan class rotasi agar panah menunjuk ke bawah
                    arrow.classList.add('rotate-90');
                } else {
                    // SAAT MENUTUP:
                    subtasks.classList.add('hidden');
                    // Hapus class rotasi agar panah kembali menunjuk ke kanan
